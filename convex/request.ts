...
export const create = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        const alreadySentRequest = await ctx.db
            .query("request")
        ...
        const alreadyReceivedRequest = await ctx.db
            .query("request")
            .withIndex("by_receiver_sender", (q) =>
        ...
        if (friends1.some((friend) => friend.user2 === receiver._id) || friends2.some((friend) => friend.user1 === receiver._id)) {
            throw new ConvexError("You are alredy friends with this user")
        }
...
export const deny = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        if (!request || request.receiver !== currentUser._id) {
            throw new ConvexError("There was an error in denying the reques!")
        }
...
export const accept = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        if (!request || request.receiver !== currentUser._id) {
            throw new ConvexError("There was an error in accepting the reques!")
        }
